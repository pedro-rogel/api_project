...
class TestStringMethods(unittest.TestCase):
    ...
    def test_007_alunos_post_sem_turmas(self):
        r = requests.post('http://localhost:9090/alunos',json={
        "nome": "Pedro",
        ...
        self.assertEqual(r.json()['erro'],'Nao ha turmas para criar alunos')


    def test_008_turmas_post_sem_professores(self):
        r = requests.post('http://localhost:9090/turmas',json={ 
            'nome':'ADS', 
            'turno':'Diurno',
            'descricao': 'Teste',
            'status': True,
        ...
        self.assertIn(r.status_code,[400,404])
        self.assertEqual(r.json(), {'erro': 'Nao ha professores para criar turmas'})


    def test_009_professores_post_criar_professor(self):
        r = requests.post('http://localhost:9090/professores',json={
            "nome": "João",
        ...
        r_lista = requests.get('http://localhost:9090/professores')
    ...
    def test_010_turmas_post_criar_turmas(self):
        ...
        r_lista = requests.get('http://localhost:9090/turmas')
    ...
    def test_011_alunos_post_criar_alunos(self):
        ...
        r_lista = requests.get('http://localhost:9090/alunos')
    ...
    def test_018_alunos_delete(self):
        ...
        requests.delete('http://localhost:9090/alunos/1')
        r_lista = requests.get('http://localhost:9090/alunos')
    ...
    def test_019_turmas_delete(self):
        r_lista = requests.get('http://localhost:9090/turmas')
        self.assertEqual(len(r_lista.json()),2)
        requests.delete('http://localhost:9090/turmas/1')
        r_lista = requests.get('http://localhost:9090/turmas')
        self.assertEqual(len(r_lista.json()),1)
    ...
    def test_020_professores_delete(self):
        r_lista = requests.get('http://localhost:9090/professores')
        self.assertEqual(len(r_lista.json()),2)
        requests.delete('http://localhost:9090/professores/1')
        r_lista = requests.get('http://localhost:9090/professores')
        self.assertEqual(len(r_lista.json()),1)
